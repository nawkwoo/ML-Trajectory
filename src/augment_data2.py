"""
Augment trajectories from results/preprocessed_data2 and save to results/augmented_data2.
Basic methods: noise, time shift, crop+resample, masking, XY rotation, same-class mixup.
Default: only quality=0 (1st) samples are augmented; originals (1st/2nd) are kept.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Augment trajectories from results/preprocessed_data2",
    )
    parser.add_argument(
        "--quality-filter",
        choices=["0", "1", "all"],
...
def main() -> None:
    ...
    base_dir = os.path.join("results", "preprocessed_data2")
    if not os.path.exists(os.path.join(base_dir, "X.npy")):
        raise FileNotFoundError("results/preprocessed_data2/X.npy not found. Run `python src/preprocess2.py` first.")
    ...
    out_dir = os.path.join("results", "augmented_data2")
