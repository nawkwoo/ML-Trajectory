"""Shared utilities for horizontal trajectory EDA/PCA (kwanwoo-style structure)."""
...
# ---------------------------------------------------------------------------
# Paths (prefer local pre-project/Data if present, otherwise top-level Data)
# ---------------------------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parent
REPO_ROOT = PROJECT_ROOT.parent

# Ensure repo root is importable so we can reach preprocess.loader
...
def _numeric_key(path: Path) -> Tuple[int, str]:
    """Sort by numeric file id if possible."""
    stem = path.stem
    try:
...
def load_all() -> List[Trajectory]:
    ...
    if not trajectories:
        raise RuntimeError("No trajectories found under the provided horizontal folders.")
    return trajectories
...
def normalize_and_pca(seq: np.ndarray, eps: float = 1e-8) -> Tuple[np.ndarray, np.ndarray, float, np.ndarray]:
    """Center → scale by mean radius → PCA align."""
    coords = np.asarray(seq, dtype=float)
...
def plot_box_and_hist(df_T: pd.DataFrame, out_dir: Path, prefix: str, title_prefix: str = "") -> None:
    ...
    for feat in IMPORTANT_FEATURES:
        ...
        ax.boxplot([data_clean.values, data_noise.values], labels=["original", "noise"], showmeans=True)
        ax.set_ylabel(feat)
        fig.tight_layout()
...
def export_raw_csv(trajs: Sequence[Trajectory], out_path: Path) -> None:
    rows = []
    for traj in trajs:
