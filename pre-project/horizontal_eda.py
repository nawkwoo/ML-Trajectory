...
OUT_INDIVIDUAL = OUT_ROOT / "individual"
...
def run_eda() -> None:
    ...
    # 개별 플롯
    for traj in trajs:
        base = f"horizontal_{traj.group}_{traj.file_id:02d}"
        plot_3d(traj.seq, title=f"{base} - 3D", save_path=OUT_INDIVIDUAL / f"{base}_3d.png")
        plot_projection(traj.seq, (0, 1), ("X", "Y"), title=f"{base} - XY", save_path=OUT_INDIVIDUAL / f"{base}_xy.png")
        plot_projection(traj.seq, (1, 2), ("Y", "Z"), title=f"{base} - YZ", save_path=OUT_INDIVIDUAL / f"{base}_yz.png")
        plot_projection(traj.seq, (0, 2), ("X", "Z"), title=f"{base} - XZ", save_path=OUT_INDIVIDUAL / f"{base}_xz.png")

    # 그룹 오버레이
    plot_group_overlay(trajs, "Original vs Noise - 3D", OUT_GROUP / "group_original_vs_noise_3d.png")
    plot_group_overlay(trajs, "Original vs Noise - XY", OUT_GROUP / "group_original_vs_noise_xy.png", axes=(0, 1))
    plot_group_overlay(trajs, "Original vs Noise - YZ", OUT_GROUP / "group_original_vs_noise_yz.png", axes=(1, 2))
    plot_group_overlay(trajs, "Original vs Noise - XZ", OUT_GROUP / "group_original_vs_noise_xz.png", axes=(0, 2))

    # 특징 표/통계 + 박스플롯/히스토그램
    df_T = save_feature_tables(trajs, OUT_FEATURES, prefix="horizontal")
    plot_box_and_hist(df_T, OUT_FEATURES, prefix="horizontal", title_prefix="")
    print(f"[INFO] Raw EDA artifacts saved to {OUT_FEATURES.parent}")
