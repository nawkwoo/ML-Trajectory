...
- 전처리 단계 (`preprocess.py`):
  1) 원점 이동(normalize_origin)
  2) 스케일 정규화(최대 거리=1)
  3) 길이 100으로 보간(resample_trajectory)
  4) `preprocessed_data/X.npy (N,100,3)`, `y.npy (N,)`, `quality.npy (N,)` 생성  
     - `quality`: 0=1st(고품질), 1=2nd(노이즈 포함)
- 실행: `python preprocess.py`

## 2) Data 증강
- 스크립트: `augment_data.py` (기본: 1st만 증강, 원본은 1st+2nd 모두 포함)
...
- 출력: `augmented_data/X.npy, y.npy, quality.npy`
- 실행: `python augment_data.py`

## 3) ML 모델 (SVM / RandomForest)
- 스크립트: `preprocessed_data/ml_model.py`
- 데이터: `augmented_data`(필수; 없으면 오류)
...
- 실행: `python preprocessed_data/ml_model.py`

## 4) DL1 모델 (GRU, 1st→2nd)
- 스크립트: `preprocessed_data/dl_model_1.py`
- 데이터: `augmented_data`(필수; 없으면 오류)
...
- 실행: `python preprocessed_data/dl_model_1.py`

## 5) DL2 모델 (GRU, k-fold + 전체 재학습)
- 스크립트: `preprocessed_data/dl_model_2.py`
- 데이터: `augmented_data`(필수; 없으면 오류)
- 전처리: 채널별 mean/std 정규화
- 평가:
...
- 실행: `python preprocessed_data/dl_model_2.py`
